  ...
  tabPanel("Inicio", 
    ...
    sidebarLayout(
      sidebarPanel(width = 3,id='sidebar',
        ...
        fileInput('file2', 'arq2',accept = c('text/csv','text/comma-separated-values','text/tab-separated-values','text/plain','.csv','.tsv', '.xlsx'), width =400),
...
server <- function(input, output, session) {
  ...
  ###Javascript que oculta a div da aba Analises###
  shinyjs::hide(id = 'Sidebar')
  shinyjs::hide(id = 'divAnalises')
  
  ###JAVASCRIPT para desabilitar o botao de calcular###
  observe({
    shinyjs::toggleState("do", !is.null(input$file1)  && !is.null(input$file2)) 
  })
  
  ###JAVASCRIPT do arquivo oculto###
